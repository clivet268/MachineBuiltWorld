...
public abstract class AbstractGearEntity extends DamagingProjectileEntity implements IProjectile {
    private static final DataParameter<Byte> CRITICAL = EntityDataManager.createKey(AbstractGearEntity.class, DataSerializers.BYTE);
    protected static final DataParameter<Optional<UUID>> field_212362_a = EntityDataManager.createKey(AbstractGearEntity.class, DataSerializers.OPTIONAL_UNIQUE_ID);
    private static final DataParameter<Float> PIERCE_LEVEL = EntityDataManager.createKey(AbstractGearEntity.class, DataSerializers.FLOAT);
    @Nullable
    private BlockState inBlockState;
    protected boolean inGround;
    protected int timeInGround;
    public AbstractGearEntity.PickupStatus pickupStatus = AbstractGearEntity.PickupStatus.DISALLOWED;
    ...
    protected AbstractGearEntity(EntityType<? extends AbstractGearEntity> type, World worldIn) {
        super(type, worldIn);
    }

    protected AbstractGearEntity(EntityType<? extends AbstractGearEntity> type, double x, double y, double z, double dx, double dy, double dz, World worldIn) {
        super(type, x, y, z, dx, dy, dz, worldIn);
        //this(type, worldIn);
        this.setPosition(x, y, z);
    }

    protected AbstractGearEntity(EntityType<? extends AbstractGearEntity> type, LivingEntity shooter, double dx, double dy, double dz, World worldIn) {
        super(type, shooter.getPosX(), shooter.getPosYEye() - (double)0.1F, shooter.getPosZ(), dx, dy, dz, worldIn);
        this.setShooter(shooter);
        if (shooter instanceof PlayerEntity) {
            this.pickupStatus = AbstractGearEntity.PickupStatus.ALLOWED;
        }
    ...
    public void writeAdditional(CompoundNBT compound) {
        ...
        compound.putInt("PierceLevel", (int)this.getPierceLevel());
    ...
    public void onCollideWithPlayer(PlayerEntity entityIn) {
        if (!this.world.isRemote && (this.inGround || this.getNoClip()) && this.arrowShake <= 0) {
            boolean flag = this.pickupStatus == AbstractGearEntity.PickupStatus.ALLOWED || this.pickupStatus == AbstractGearEntity.PickupStatus.CREATIVE_ONLY && entityIn.abilities.isCreativeMode || this.getNoClip() && this.getShooter().getUniqueID() == entityIn.getUniqueID();
            if (this.pickupStatus == AbstractGearEntity.PickupStatus.ALLOWED && !entityIn.inventory.addItemStackToInventory(this.getBulletStack())) {
                flag = false;
            }
    ...
    public void setPierceLevel(int level) {
        this.dataManager.set(PIERCE_LEVEL, (float)level);
    }
    ...
    public int getPierceLevel() {
        return (int)this.dataManager.get(PIERCE_LEVEL).intValue();
    }
    ...
    public static enum PickupStatus {
        ...
        public static AbstractGearEntity.PickupStatus getByOrdinal(int ordinal) {
