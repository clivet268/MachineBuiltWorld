...
public class TBatteryTile extends TileEntity implements ITickableTileEntity {
    ...
    public void tick() {
        ...
        if (counter > 0) {
            counter--;
            if (counter <= 0) {
                energyStorage.addEnergy(TBATTERY_GENERATE.get());
            }
            markDirty();
        }
                ...
                if (te != null) {
                    boolean doContinue = te.getCapability(CapabilityEnergy.ENERGY, direction).map(handler -> {
                                if (handler.canReceive()) {
                                    int received = handler.receiveEnergy(Math.min(capacity.get(), TBATTERY_SEND.get()), false);
