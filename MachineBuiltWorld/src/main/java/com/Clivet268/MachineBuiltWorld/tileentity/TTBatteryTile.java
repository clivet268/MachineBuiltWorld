...
public class TTBatteryTile extends TileEntity implements ITickableTileEntity {
    ...
    public void tick() {
        ...
        if (counter > 0) {
            counter--;
            if (counter <= 0) {
                energyStorage.addEnergy(TTBATTERY_GENERATE.get());
            }
            markDirty();
        }
